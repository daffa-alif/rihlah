...
exports.completeTrip = functions.https.onCall(async (data, context) => {
    ...
    if (trip.status !== 'inTrip') {
        throw new functions.https.HttpsError('failed-precondition', `Cannot complete trip in status: ${trip.status}`);
    }
    if (trip.driverId !== driverId) {
        throw new functions.https.HttpsError('permission-denied', 'You are not the assigned driver for this trip');
    }
    // Recompute fare from actual values
    const fare = (0, helpers_1.calculateFare)(trip.distanceKm || 0, trip.durationMin || 0, trip.serviceType);
    // Update trip
    const completedAt = (0, helpers_1.serverTimestamp)();
