...
class DriverDetailScreen extends StatefulWidget {
  const DriverDetailScreen({super.key, required this.uid});
  final String uid;
...
class _DriverDetailScreenState extends State<DriverDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final u = _user;
    if (u == null) {
      return const AdminEmptyState(message: 'Driver tidak ditemukan.');
    }
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _InfoCard(title: 'Kendaraan', rows: [
                ('Tipe', u.vehicleType == 'bike' ? 'Motor' : 'Mobil'),
                ('Plat Nomor', u.plate ?? '—'),
                ('NIK (masked)', u.ktpNumberMasked?.isNotEmpty == true
                    ? '•••• •••• ${u.ktpNumberMasked}' : '—'),
              ]),
              _InfoCard(title: 'Kepatuhan', rows: [
                ('BPJS Ketenagakerjaan', _statusLabel(u.bpjsKt)),
                ('BPJS Kesehatan', _statusLabel(u.bpjsKs)),
                ('Asuransi Perpres 27/2026', _statusLabel(u.bpjsInsurance)),
              ]),
              _InfoCard(title: 'Performa', rows: [
                ('Rating', u.ratingAvg?.toStringAsFixed(2) ?? '—'),
                ('Saldo (Firestore)', u.balanceIdr != null ? formatIdr(u.balanceIdr!) : '—'),
              ]),
            ],
          ),
          ...
          StreamBuilder<List<TripModel>>(
            stream: FirestoreService.instance.driverTripsStream(widget.uid, limit: 100),
                ...
                child: SingleChildScrollView(
                  ...
                  child: DataTable(
                    columns: const [
                      ...
                      DataColumn(label: Text('Bagian Driver'), numeric: true),
                    ...
                    rows: trips.map((t) {
                      ...
                      return DataRow(cells: [
                        ...
                        DataCell(Text(formatIdr(t.driverEarns))),
